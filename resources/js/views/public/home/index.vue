    ...
    <div class="home-page">
        <section class="relative overflow-hidden">
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24 relative">
                <div class="max-w-4xl">
                    <p class="uppercase tracking-[0.2em] text-xs sm:text-sm text-white/80 mb-4 reveal-1">Plataforma de
                        eventos</p>
                    ...
                    <p class="text-base sm:text-lg text-white/90 max-w-2xl mb-8 reveal-2">
                        Conciertos, festivales, deporte y cultura en una sola web. Busca por ciudad, fecha o categoría y
                        reserva en segundos.
                    </p>

                    <div class="reveal-3 rounded-2xl p-3 sm:p-4 max-w-3xl border backdrop-blur-sm"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-2 sm:gap-3">
                            <div class="md:col-span-6">
                                <InputText v-model="searchText" type="text" placeholder="Artista, recinto o evento"
                                    class="w-full" />
                            </div>
                            <div class="md:col-span-3">
                                <Select v-model="selectedCity" :options="cityOptions" optionLabel="label"
                                    optionValue="value" placeholder="Ciudad" class="w-full" />
                            </div>
                            <div class="md:col-span-3">
                                <Button label="Buscar" icon="pi pi-search"
                                    ...
                                    }" :loading="isLoadingEvents" @click="applySearch" />
                            </div>
                    ...
                    <div class="flex flex-wrap gap-2 mt-6 reveal-3">
                        <Tag v-for="chip in quickFilters" :key="chip.value" :value="chip.label"
        ...
        <section id="categorias" class="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-14">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-2xl sm:text-3xl font-bold text-surface-900 dark:text-surface-0">Explora por categoría
                </h2>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
                <button v-for="cat in categories" :key="cat.id"
                    ...
                    <p class="font-semibold text-sm sm:text-base"
                        :class="isDarkTheme ? 'text-slate-100' : 'text-slate-900'">{{ cat.name }}</p>
                </button>
            </div>
        </section>
        ...
        <section id="eventos" class="container mx-auto px-4 sm:px-6 lg:px-8 pb-14 sm:pb-16">
            <div class="flex items-center justify-between mb-6">
                ...
                <Button label="Ver todos" text class="hover:!bg-blue-100" :pt="{ label: { class: '!text-blue-600' } }"
                    @click="router.push('/events')" />
            </div>

            <div v-if="isLoadingFeatured" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <article v-for="n in 3" :key="`skeleton-${n}`"
            ...
            <div v-else-if="featuredCards.length"
                class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
                <article v-for="event in featuredCards" :key="event.id"
                    ...
                    <div class="bg-gradient-to-r from-blue-700 to-blue-500 p-4 text-white">
                        <h3 class="text-2xl font-bold mb-2 line-clamp-2 tracking-tight">{{ event.title }}</h3>
                    ...
                    <div class="p-4 flex-1 flex flex-col">
                        <p class="mb-3 line-clamp-2 leading-relaxed"
                        ...
                        <div class="space-y-2 text-sm mb-4" :class="isDarkTheme ? 'text-slate-200' : 'text-slate-700'">
                            <div class="flex items-center gap-2">
                                ...
                                <span>{{ event.startDateLabel }}</span>
                            </div>
                            <div class="flex items-center gap-2">
            ...
            <div v-else
                class="rounded-2xl border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900/40 p-8 text-center">
